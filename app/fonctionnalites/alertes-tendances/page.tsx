        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <ScrollReveal delay={200} className="space-y-6 text-base sm:text-lg text-slate-650 dark:text-slate-355 leading-relaxed">
              <p>
                Creatabl surveille en permanence les hashtags, sons et sujets qui explosent sur chaque réseau social. Dès qu&apos;une tendance pertinente pour votre niche émerge, vous recevez une notification instantanée.
              </p>
              <p>
                Configurez vos mots-clés et secteurs d&apos;activité une seule fois. L&apos;IA filtre le bruit pour ne vous alerter que sur les opportunités réellement pertinentes pour votre audience.
              </p>
              <p>
                Réagissez en temps réel là où vos concurrents attendent le lendemain. C&apos;est souvent dans les premières heures qu&apos;une tendance génère le plus d&apos;engagement.
              </p>
            </ScrollReveal>
          </div>
                ...
                <div className="bg-white dark:bg-[#121118] border border-slate-200/60 dark:border-purple-950/40 rounded-2xl p-6 space-y-4 shadow-xl">
                  <div className="flex justify-between items-center pb-3 border-b border-slate-100 dark:border-purple-950/20">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-slate-800 dark:text-slate-200">Tendances Surveillées</span>
                    </div>
                  ...
                  <div className="space-y-3.5">
                    {[
                      ...
                      <div key={i} className="p-3 bg-slate-50 dark:bg-[#181622] border border-slate-100 dark:border-purple-950/10 rounded-xl flex justify-between items-start gap-4">
                        <div>
                          <span className="text-[9px] text-[#7225E3] font-bold block mb-0.5">{trend.type}</span>
                          <span className="text-xs font-bold text-slate-800 dark:text-slate-200">{trend.tag}</span>
                          <span className="text-[10px] text-slate-400 block mt-0.5">{trend.desc}</span>
                        </div>
                        <span className="text-xs font-extrabold text-red-500 bg-red-50 dark:bg-red-950/20 px-2 py-0.5 rounded-full shrink-0">
                          {trend.growth}
                        </span>
                      </div>
                    ))}
